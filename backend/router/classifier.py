...
class DocumentRouter:
    ...
    def _load_layoutlmv3(self) -> None:
        ...
        try:
            ...
            if self.model_path and self.model_path.exists():
                ...
                env_model = os.getenv("CRED_EXA_LAYOUTLMV3_MODEL")
                if env_model and Path(env_model).exists():
                    checkpoint = env_model

            if checkpoint:
                self.processor = LayoutLMv3Processor.from_pretrained(checkpoint)
                self.model = LayoutLMv3Model.from_pretrained(checkpoint)
            else:
        ...
        for category, keywords in self.taxonomy.items():
            for keyword in keywords:
                ...
                if keyword in search_blob:
                    ...
                    token_hits = sum(1 for token in keyword.split() if token in search_blob)
    ...
    def _layout_signal(self, payload: Mapping[str, Any]) -> Dict[str, float]:
        pages = payload.get("pages", []) or []
        if not pages or self.processor is None or self.model is None or torch is None:
            return {}
        ...
        try:
            ...
            energy = float(torch.linalg.norm(pooled).item())
            return {category: energy / (index + 1) for index, category in enumerate(self.taxonomy)}
        except Exception:
            return {}
    ...
    def classify_document(self, payload: Mapping[str, Any]) -> str:
        ...
        combined = {category: keyword_scores.get(category, 0.0) + (layout_scores.get(category, 0.0) * 0.05) for category in self.taxonomy}
