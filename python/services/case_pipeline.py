...
class CasePipeline:
    def __init__(self, base_dir: Path, db_path: Path, documenttypes_path: Path):
    ...
    def _insert_flag(self, conn: sqlite3.Connection, document_id: str | None, flag: Dict[str, Any]) -> None:
        conn.execute(
            "INSERT INTO flags (id, document_id, layer, finding, severity, score) VALUES (?, ?, ?, ?, ?, ?)",
            (
                str(uuid.uuid4()),
                document_id,
    ...
    def process_case(self, case_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            documents = conn.execute("SELECT * FROM documents WHERE case_id = ? ORDER BY file_name", (case_id,)).fetchall()
            document_results: List[Dict[str, Any]] = []
            doc_entities: List[Dict[str, Any]] = []
            all_flags: List[Dict[str, Any]] = []

            conn.execute("UPDATE cases SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", ("processing", case_id))
            conn.execute("DELETE FROM flags WHERE document_id IN (SELECT id FROM documents WHERE case_id = ?)", (case_id,))
            conn.execute("DELETE FROM flags WHERE document_id IS NULL AND layer IN ('Cross-Doc Consistency', 'ML Anomaly')")
            ...
            for document in documents:
                ...
                for flag in flags:
                    self._insert_flag(conn, document["id"], flag)
            ...
            for flag in cross_doc_flags:
                self._insert_flag(conn, None, flag)
            ...
            conn.execute(
                "UPDATE cases SET status = ?, risk_score = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                (scoring["status"], scoring["risk_score"], case_id),
            )
            conn.execute(
                "INSERT INTO audit_log (id, case_id, action, details) VALUES (?, ?, ?, ?)",
